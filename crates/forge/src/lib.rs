...
fn run_single_test(
    ...
    tokio::task::spawn(async move {
        tokio::select! {
            ...
            result = blocking_run_from_test(vec![], case.clone(),runner,  get_fork_state_reader(&runner_config.workspace_root, runner_config.fork_targets.as_slice(), &case.fork_config)?, runner_params.clone(), send.clone(), send_shut_down.clone() ) => {
                match result? {
                    Ok(result) => {
                        if exit_first {
